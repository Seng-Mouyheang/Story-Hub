...
const COLLECTION_NAME = "momentComments";
...
const createComment = async (commentData) => {
  ...
  const momentId = new ObjectId(commentData.momentId);
  ...
  try {
    await session.withTransaction(async () => {
      ...
      const momentUpdate = await momentsCollection.updateOne(
        {
          _id: momentId,
          expiresAt: { $gt: now },
...
// Get comments for a moment with pagination and like status
const getCommentsByMoment = async (
  momentId,
  currentUserId,
  limit = 10,
  cursor = null,
...
const getCommentById = async (id, currentUserId = null) => {
  ...
  if (currentUserId) {
    const like = await db.collection("momentCommentLikes").findOne({
      userId: new ObjectId(currentUserId),
        ...
        [
          {
            $set: {
              commentCount: {
                $max: [{ $subtract: ["$commentCount", totalDeleted] }, 0],
              },
            },
          },
        ],
